				...
				<form id="editAssignInstructorToClass" action="<?php echo base_url('Admin/editAssignInstructor/'.$Id); ?>" method="POST" enctype="multipart/form-data">
					<div class="card card-form">
						...
						<div class="card-header-text-form bg-rose">
							<h4 class="title">Edit Assign Instructor to Class</h4>
						</div>
										 ...
										 <option value=""></option>
										  <!------appended data from ajax------>
										   ...
										   if($instructor_data['ClassType'] == 1){
											...
											?>
										   <option value="<?php echo $ind_data['ClassId']; ?>" <?php echo ($ind_data['ClassId'] == $instructor_data['ClassName']) ? 'selected': ''; ?>><?php echo $ind_data['ClassName']." From ".$startTimeName." - ".$endTimeName; ?></option>
										   ...
										   if(!empty($assign_corp_class)){
												foreach($assign_corp_class as $key => $data){
													...
													$endTimeName = $end['TimeName'];
											?>
										   <option value="<?php echo $data['ClassId']; ?>" <?php echo ($data['ClassId'] == $instructor_data['ClassName']) ? 'selected': ''; ?>><?php echo $data['ClassName']." From ".$startTimeName." - ".$endTimeName; ?></option>
										   <?php }}} ?>
										</select>
									</div>
								</div>
