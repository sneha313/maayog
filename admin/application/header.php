                        ...
                        <ul class="nav navbar-nav navbar-right">
                            <li class="dropdown NotificationIcon">
                                <a href="javascript:void(0);" class="dropdown-toggle NotificationDetail" id="notificationDetail" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">
									...
									<?php //$result = $this->Admin_Model->get_notification_count(); if($result > 0){ ?>
                                    <span class="notification NumIcon" data-id='' style="display:none;"><?php //echo $result; ?></span>
                                    <p class="hidden-lg hidden-md">Notifications</p>
									<?php //} ?>
                                </a>
                                <ul class="dropdown-menu">
								<?php $notification=$this->Admin_Model->get_latest_notification();
                            ...
                            <li>
								<div class="dropdown" style="float:right;">
								  <img class="img-responsive user-profile" src="<?php echo base_url('assets/img/faces/user.jpg'); ?>">
								  <div class="dropdown-content">
										<div class="row ProfileHeader">
											<div class="col-xs-4 col-sm-4 col-md-4 col-lg-4 col-xl-4" style="float:left;">
												<img class="img-responsive img-circle profile-picture" src="<?php echo base_url('assets/img/faces/user.jpg'); ?>">
											</div>
											<div class="col-xs-8 col-sm-8 col-md-8 col-lg-8 col-xl-8 text-center">
												<strong class="text-user-name"><?php echo (empty($this->session->userdata('FullName')))? 'User': $this->session->userdata('FullName');?></strong>
												<p class="text-user-ind"><small><?php echo (!empty($this->session->userdata('Role'))&& $this->session->userdata('Role') =='3')? '( Admin )': '( User )';?></small><br/><span class="text-purple"><?php echo (empty($this->session->userdata('EmailAddress')))? 'User': $this->session->userdata('EmailAddress');?></span></p>
											</div>	
										</div>
