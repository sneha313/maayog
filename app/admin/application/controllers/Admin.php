		...
		if(!empty($classList)){
			foreach($classList as $list){
				...
				if(!empty($res)){
					...
					foreach($res as $user){
						...
						$user['className'] = $classDetail['ClassName'];
						$user['reason'] = $this->input->post('Reason');
						$user['date'] = $this->input->post('ClickDate');
						$user['title'] = 'Cancelled Class';
						$user['content']='<span class="promotion-code" style="text-overflow: ellipsis;font-size: 14px;font-style: italic;line-height: 35px;color: #136193;font-weight: 400;"> Dear, <br> A class Has been cancelled on '.$ClickDate.'. Find Detail below:';
						// $message = $this->load->view('email-template',$user,true);
						$message = $this->load->view('class-cancel-template',$user,true);
						// $status = false;
			...
			if(!empty($res)){
				...
				foreach($res as $user){
					...
					$className = $classDetail['ClassName'];
					$user['className'] = $classDetail['ClassName'];
					$user['rescheduleDate'] = $this->input->post('RescheduleDate');
					$user['reason'] = $reason;
					$user['title'] = 'Rescheduled Class';
					$user['content']='<span class="promotion-code" style="text-overflow: ellipsis;font-size: 14px;font-style: italic;line-height: 35px;color: #136193;font-weight: 400;"> Dear, <br> Your class has been rescheduled on '.$this->input->post('RescheduleDate').'.Find detail below:';
					echo $message = $this->load->view('class-reschedule-template',$user,true);
					if(isset($emailAddress) && !empty($emailAddress)) {
			...
			if(!empty($resp)){
				...
				foreach($resp as $userDetail){
					...
					$userDetail['Reason'] = $this->input->post('Reason');
					$userDetail['ClickDate'] = $this->input->post('ClickDate');
					$userDetail['content']='<span class="promotion-code" style="text-overflow: ellipsis;font-size: 14px;font-style: italic;line-height: 35px;color: #136193;font-weight: 400;"> Dear, <br> A new event created. Find detail below:';
					$message = $this->load->view('event-name-template',$userDetail,true);
					if(isset($emailAddress) && !empty($emailAddress)) {
...
// exit;
	...
	public function viewPayment($Id) {	
		...
		$result['Payment'] = $this->Admin_Model->getPayment_ById($Id);
